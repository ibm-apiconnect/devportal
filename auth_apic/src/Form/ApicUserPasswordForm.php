...
class ApicUserPasswordForm extends UserPasswordForm {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    if(sizeof($user_registries) > 1) {
      $other_registries = array_diff_key($user_registries, array($chosen_registry->getUrl() => $chosen_registry));
    }
    ...
    if(!empty($other_registries)) {
      ...
      foreach($other_registries as $other_registry) {
        ...
        if($other_registry->getRegistryType() === 'google') {
          $button['#prefix'] = '<a class="registry-button google-button button" href="' . $redirect_with_registry_url . $other_registry->getUrl() . '" title="' . $this->t("Use @ur", array('@ur' => $other_registry->getTitle())) . '">
          ...
          $button['#prefix'] = '<a class="registry-button github-button button" href="' . $redirect_with_registry_url . $other_registry->getUrl() . '" title="' . $this->t("Use @ur", array('@ur' => $other_registry->getTitle())) . '">
          ...
          $button['#prefix'] = '<a class="registry-button generic-button button" href="' . $redirect_with_registry_url . $other_registry->getUrl() . '" title="' . $this->t("Use @ur", array('@ur' => $other_registry->getTitle())) . '">
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($result !== NULL && ((is_a($result, 'Drupal\ibm_apim\Rest\RestResponse') && $result->getCode() !== 500) || (isset($result->code) && $result->code !== 500))) {
      drupal_set_message(t('If the account exists, an email has been sent with further instructions to reset the password.'));
      $form_state->setRedirect('user.page');
    } else {
      $this->logger->info('ELSE: result:' . serialize($result));
    }
