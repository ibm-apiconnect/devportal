...
sed -i 's/\*:4443/*:443/' /var/aegir/config/server_master/nginx/vhost.d/localhost
sed -i 's+127.0.0.1:9000+unix:/var/run/php7.1-fpm.sock+' /var/aegir/config/includes/nginx_vhost_common.conf
(cat /etc/nginx/nginx.conf;echo;echo "user aegir;" ) > /etc/nginx/nginx.conf.tmp
mv -f /etc/nginx/nginx.conf.tmp /etc/nginx/nginx.conf

(cat /etc/php/7.1/fpm/pool.d/www.conf;echo;echo "user=aegir") > /etc/php/7.1/fpm/pool.d/www.conf.bak
mv -f /etc/php/7.1/fpm/pool.d/www.conf.bak /etc/php/7.1/fpm/pool.d/www.conf
sed -i -e 's/^listen\.owner.*/listen.owner = aegir/' -e 's/^listen\.group.*/;listen.group = aegir/' /etc/php/7.1/fpm/pool.d/www.conf

sed -i -e 's/4443/443/' -e '/ssl_dhparam/d' -e 's/gulag/limreq/' /web/pre.d/nginx_ssl.conf
...
(su aegir <<- EOF
  ...
  . $NVM_DIR/nvm.sh
  nvm use --lts
  . modules/apictest/setupbehat.sh localhost TRUE
...
then
  echo "Dumping dblog"
  su aegir -c "drush @localhost watchdog-show --count=1000 --extended" >/tmp/watchdog/watchdog.log 2>&1
