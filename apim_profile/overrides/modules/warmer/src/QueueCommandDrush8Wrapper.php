...
class QueueCommandDrush8Wrapper
{
  ...
  public function run($name, $options = ['time-limit' => self::REQ])
  {
    ...
    $this->logger->info(dt('Processed @count items from the @name queue in @elapsed sec.', ['@count' => $count, '@name' => $name, '@elapsed' => round($elapsed, 2)]));
  }
  ...
  public function delete($name)
  {
    $queue = $this->getQueue($name);
    $queue->deleteQueue();
    $this->logger->info(dt('All items in @name queue deleted.', ['@name' => $name]));
  }
