...
class ApicTaxonomy {
  ...
  public function get_taxonomies_from_categories($categories = array()) {
    ...
    $create_enabled = $config->get('categories')['create_taxonomies_from_categories'];
    $tids = array();
  ...
  private function create_taxonomy_from_path($path) {
    ...
    if ($create_enabled) {
      ...
      if (count($pathParts) < 20) {
        ...
        $this->generate_tree(array('tags' => $taxonomy));
      }
    }
   ...
   * @param $trees
   */
  private function generate_tree($trees) {
   ...
   */
  private function create_vocabulary_tree($vocabulary_name = 'tags', $vocabulary_tree) {
   ...
   * @param int $parent
   */
  private function create_one_level_tree($vocabulary_tree, $vocabulary, $parent = 0) {
    ibm_apim_entry_trace(__CLASS__ . '::' . __FUNCTION__, NULL);
    $weight = 0;
  ...
  private function get_vocabulary_by_name($vocabulary_name) {
    ...
    if (isset($vocabularies[$vocabulary_name])) {
      $returnValue = $vocabularies[$vocabulary_name];
    }
    else {
      // Return null when non-existent vocabulary.
      $returnValue = NULL;
    }
    ibm_apim_exit_trace(__CLASS__ . '::' . __FUNCTION__, NULL);
    return $returnValue;
