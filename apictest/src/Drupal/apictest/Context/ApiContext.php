...
class ApiContext extends RawDrupalContext {
   ...
   */
  public function iPublishAnApiWithTheNameIdAndCategories($name, $id, $categories) {
    ...
    if (!\Drupal::config('ibm_apim.settings')->get('categories')['create_taxonomies_from_categories']) {
      ...
      \Drupal::service('config.factory')->getEditable('ibm_apim.settings')->set('categories.create_taxonomies_from_categories', TRUE)->save();
    }
   ...
   */
  public function iShouldHaveAnApiWithTheNameIdAndCategories($name, $id, $categories) {
    ...
    if (isset($results) && !empty($results)) {
      ...
      if ($api->get('api_id')->value === $id && $api->get('api_xibmname')->value === $name) {
      ...
      if ($terms = taxonomy_term_load_multiple_by_name("Sport", "tags")) {
        ...
        $terms = \Drupal::entityTypeManager()->getStorage('taxonomy_term')->loadTree("tags", reset($terms)->get('tid')->value);
   ...
   */
  public function iPublishAnApiWithTheNameAndCategoriesAndCreateTaxonomiesFromCategoriesIsFalse($name, $categories) {

    $object = $this->createApiObject($name, '12345', $categories);

    if (\Drupal::config('ibm_apim.settings')->get('categories')['create_taxonomies_from_categories']) {
      print("Setting 'create_taxonomies_from_categories' config to false. ");
      \Drupal::service('config.factory')->getEditable('ibm_apim.settings')->set('categories.create_taxonomies_from_categories', FALSE)->save();
    }
   ...
   */
  public function iShouldHaveAnApiWithNameAndNoTaxonomiesForTheCategories($name, $categories) {
    $results = $this->searchForApiByTitle($name);
    ...
    if (isset($results) && !empty($results)) {
      ...
      if ($api->get('api_xibmname')->value === $name) {
   ...
   */
  public function iPublishAnApiWithTheNameAndAutotagWithPhaseIsTrue($name, $phase) {
    $object = $this->createApiObject($name, '12345', NULL, $phase);

    if (!\Drupal::config('ibm_apim.settings')->get('autotag_with_phase')) {
   ...
   */
  public function iShouldHaveAnApiWithTheNameTaggedWithThePhase($name, $phase) {
    $results = $this->searchForApiByTitle($name);
   ...
   */
  public function iPublishAnApiWithTheNameAndAutocreateApiforumIsTrue($name) {
    $object = $this->createApiObject($name);

    if (!\Drupal::config('ibm_apim.settings')->get('autocreate_apiforum')) {
   ...
   */
  public function iShouldHaveAnApiWithTheNameAndAForumWithTheName($name) {
    $results = $this->searchForApiByTitle($name);
   ...
   */
  public function iPublishAnApiWithTheNameAndAutocreateApiforumIsFalse($name) {
    $object = $this->createApiObject($name);
   ...
   */
  public function iShouldHaveAnApiNameAndNoForum($name) {
    $results = $this->searchForApiByTitle($name);
   ...
   */
  public function iPublishAnApiWithTheNameAndId($name, $id) {
   ...
   */
  public function iUpdateTheApiToHaveTheUrl($url) {
   ...
   */
  public function iShouldHaveAnApiWithTheIdAndUrl($id, $url) {
    ...
    if (isset($results) && !empty($results)) {
      ...
      if ($api->api_id->value === $id && $api->apic_url->value === $url) {
   ...
   */
  public function iPublishAnApiWithTheName($name) {
    $object = $this->createApiObject($name);
   ...
   */
  public function iDeleteTheApiWithTheName($name) {
    $results = $this->searchForApiByTitle($name);
   ...
   */
  public function iShouldNoLongerHaveAnApiWithTheName($name) {
   ...
   */
  public function iShouldHaveAnApiNameVisibleBy($name, $switchto) {
   ...
   * @return array
   */
  private function createApiObject($name, $id = '12345', $categories = NULL, $phase = NULL) {

    $object = array();
    $object['consumer_api'] = array();
    $object['consumer_api']['info'] = array();
    ...
    $object['consumer_api']['x-ibm-configuration'] = array();
    $object['consumer_api']['x-ibm-configuration']['type'] = 'rest';

    if ($categories) {
      $object['consumer_api']['x-ibm-configuration']['categories'] = array($categories);
    }
    if ($phase) {
