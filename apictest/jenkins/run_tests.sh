...
su aegir -c "drush @localhost en potx"
su aegir -c "drush @localhost potx --modules=$MODULE_NAME"
...
if [ -d modules/$MODULE_NAME/features ]; then
  ...
  su aegir -c "export NVM_DIR=/root/.nvm && . $NVM_DIR/nvm.sh	&& nvm use 6.3.0 && . modules/apictest/setupbehat.sh localhost TRUE && ./runbehat.sh $TESTS_TO_RUN" && RC=$? || RC=$?
